...
for x in range(a):
    try:
        key,items=random.choice(list(code.items()))
        result = tr.translate(word, src=itemlis[num-1], dest=items).text
        #print(key,items)
        #print(itemlis)
        word=result
        num+=1
        itemlis.append(items)
        lists.append(key)
        print(result)
    except:
        x-=1
        continue
result = tr.translate(word, src=itemlis[num-1], dest="ja").text
lists.append("日本語")
print(lists)
print(result)
